...
RVec build_A_diagonal_TM(const YeeGrid& g) {
    ...
    for (Int k = 0; k < N2; ++k) {
        ...
        // Blocks Hx, Hy
        A[static_cast<std::size_t>(TM::BLK_HX * N2 + k)] = PhysConst::mu0;
        A[static_cast<std::size_t>(TM::BLK_HY * N2 + k)] = PhysConst::mu0;
        // Block Ez: ε∞(r)
...
RVec build_A_diagonal_TE(const YeeGrid& g) {
    ...
    for (Int k = 0; k < N2; ++k) {
        const std::size_t ks = static_cast<std::size_t>(k);
        A[static_cast<std::size_t>(TE::BLK_HZ * N2 + k)] = PhysConst::mu0;
